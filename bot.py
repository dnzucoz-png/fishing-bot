  ...
  async def evaluate_biting(self, fish_type, region, hour, day_offset):
    ...
    try:
      ...
      return {
          ...
          "expert_commentary": (
              "Чудові погодні умови для риболовлі. Риба проявляє активність у"
              " прибережній зоні."
          ),
      }
    except Exception as e:
...
async def cmd_start(message: Message):
  ...
  await message.answer(
      "📍 Поточна область: **Дніпропетровська**\nОберіть область для риболовлі"
      " або почніть вибір риби:",
  ...
  fish_keyboard = InlineKeyboardMarkup(
      inline_keyboard=[
          [
              InlineKeyboardButton(text="Лящ", callback_data="fish_Лящ"),
...
async def handle_change_region(callback: CallbackQuery):
  keyboard_regions = InlineKeyboardMarkup(
      inline_keyboard=[
          [
              InlineKeyboardButton(
                  text="Дніпропетровська", callback_data="region_Дніпропетровська"
              )
          ],
          [
              InlineKeyboardButton(
                  text="Київська", callback_data="region_Київська"
              )
          ],
          [
              InlineKeyboardButton(
                  text="Львівська", callback_data="region_Львівська"
              )
          ],
          [
              InlineKeyboardButton(
                  text="Одеська", callback_data="region_Одеська"
              )
          ],
      ]
  )
  await callback.message.answer(
      "Оберіть область з решти списку:", reply_markup=keyboard_regions
  )
...
async def handle_region_selected(callback: CallbackQuery):
  ...
  await callback.message.answer(
      f"✅ Успішно обрано область: **{region_name}**.\nТепер виберіть рибу:",
...
async def handle_fish_callback(callback: CallbackQuery):
  ...
  hours_keyboard = InlineKeyboardMarkup(
      inline_keyboard=[
          [
              InlineKeyboardButton(
                  text="🌅 Світанок (06:00)", callback_data="hour_6"
              )
          ],
          [
              InlineKeyboardButton(
                  text="☀️ День (12:00)", callback_data="hour_12"
              )
          ],
          [
              InlineKeyboardButton(
                  text="🌇 Захід сонця (20:00)", callback_data="hour_20"
              )
          ],
      ]
  )
  ...
  await callback.message.answer(
      f"📍 Область: {current_region} | Вибрано рибу: {fish_type}.\nОберіть час"
      " доби:",
...
async def handle_hour_callback(callback: CallbackQuery):
  ...
  state = user_states.get(
      user_id, {"region": "Дніпропетровська", "fish": "Лящ", "day_offset": 0}
  )
  ...
  if region not in REGIONS:
    await callback.message.answer(
        f"Помилка: не знайдено координати для регіону {region}."
    )
    await callback.answer()
    return
  ...
  if not result:
    await callback.message.answer(
        "Не вдалося отримати метеодані. Перевірте з'єднання або спробуйте пізніше."
    )
    await callback.answer()
    return
  ...
  response_text = (
      ...
      f"🌡 Температура: {result['temperature']}°C (води:"
      f" ~{result['water_temp']}°C)\n"
      f"🌀 Тиск: {result['pressure_mm']} мм рт. ст. |"
      f" {result['pressure_stability']}\n"
      f"💨 Вітер: {result['wind_ms']} м/с ({result['wind_dir']}) | 🌧 Опади:"
      f" {result['precipitation']} мм\n\n"
      f"⭐ **Оцінка кльову:** {result['stars']}/5"
...
async def main():
  print("Бот запущено...")
  # Запускаємо фоновий вебсервер для Render
  asyncio.create_task(web_server())

  # Очищуємо старі вебхуки та сесії, щоб уникнути TelegramConflictError
  await bot.delete_webhook(drop_pending_updates=True)

  # Запуск самого оновлення (polling)
